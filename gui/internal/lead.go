...
{CLI} wait <agent1> [agent2 ...]             비동기 작업 완료 대기 (sleep 대신 사용)
...
6. 개발 태스크이면:
   ...
   c. 의존성이 없는 작업은 --async로 병렬 실행하고, {CLI} wait로 완료를 기다리세요 (sleep 사용 금지)
...
- 절대 sleep을 사용하지 마세요. 비동기 작업 완료 대기는 반드시 {CLI} wait를 사용하세요
- 절대 sleep && {CLI} status 패턴을 사용하지 마세요
{SESSION}
[사용자 요청]
{INPUT}`
